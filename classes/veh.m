...
%             obj.sus_rr.q_val = q_tm1([10 12])';
            ...
            v_rl_cWH_dq_p = 0;
            v_rr_cWH_dq_p = 0;
            v_angp_dqp_p  = squeeze(num2cell(zeros(3,1,numel(q)),[1 2]));
            e_angp_dqp_p  = squeeze(num2cell(zeros(3,1,numel(q)),[1 2]));
            e_angpp       = 0;
            for j=1:numel(q)
                v_fl_cWHpp    = v_fl_cWHpp    + v_fl_cWH_ddq{j} * qp(j) * qp;
                v_fr_cWHpp    = v_fr_cWHpp    + v_fr_cWH_ddq{j} * qp(j) * qp;
                v_rl_cWHpp    = v_rl_cWHpp    + v_rl_cWH_ddq{j} * qp(j) * qp;
                v_rr_cWHpp    = v_rr_cWHpp    + v_rr_cWH_ddq{j} * qp(j) * qp;
                e_T_v_p       = e_T_v_p       + e_T_v_dq{j}     * qp(j);
                v_fl_cWH_dq_p = v_fl_cWH_dq_p + v_fl_cWH_ddq{j} * qp(j);
                v_fr_cWH_dq_p = v_fr_cWH_dq_p + v_fr_cWH_ddq{j} * qp(j);
                v_rl_cWH_dq_p = v_rl_cWH_dq_p + v_rl_cWH_ddq{j} * qp(j);
                v_rr_cWH_dq_p = v_rr_cWH_dq_p + v_rr_cWH_ddq{j} * qp(j);
                for i=1:numel(q)
                    v_angp_dqp_p{i}  = v_angp_dqp_p{i}  + diff2(v_angp_dqp{i},q(j)) * qp(j);
                    e_angp_dqp_p{i}  = e_angp_dqp_p{i}  + diff2(e_angp_dqp{i},q(j)) * qp(j);
                end
                e_angpp      = e_angpp      + e_angp_dq{j}  * qp(j);
            end
...
%             [M,F] = massMatrixForm(Lagrange_sym,q)
